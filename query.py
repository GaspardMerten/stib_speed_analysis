...
def get_average_speed_for(client, line_id: str, points_tuple: List[str], min_date: date,
    ...
    query = f"""WITH entries AS (
        ...
        FROM [table] 
        WHERE dayofweek(make_timestamp(timestamp*1000000)) IN ({', '.join(map(str, selected_days))}) AND hour(make_timestamp(timestamp*1000000)) >= {start_hour} AND hour(make_timestamp(timestamp*1000000)) <= {end_hour}
    ), filtered_entries AS (
        SELECT 
            *,
    ...
    SELECT  lineId, directionId, pointId, avg(speed) * 3.6, count(*) as count, {aggregation.format(date="make_timestamp(timestamp*1000000)")} as agg
    FROM speedTable
    WHERE speed > 0 
    ...
    results = client.advanced_query(
        ...
        datetime(max_date.year, max_date.month, max_date.day) + pd.Timedelta(days=1)
    )
